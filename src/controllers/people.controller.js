...
export async function searchPeopleForEmail(req, res) {
  try {
      ...
      const peopleStartsWithQuery = await People.findAll({
        where: sequelize.where(sequelize.fn('LOWER', sequelize.col('telefono_contacto')), {
          [Op.like]: `${queryLower}%`  
        }),
        order: [
          sequelize.literal(`LOWER(telefono_contacto) LIKE '${queryLower}%' DESC`)
        ]
      });
      ...
      const peopleContainsQuery = await People.findAll({
        where: {
          [Op.and]: [
            sequelize.where(sequelize.fn('LOWER', sequelize.col('telefono_contacto')), {
              [Op.notLike]: `${queryLower}%`
            }),
            sequelize.where(sequelize.fn('LOWER', sequelize.col('telefono_contacto')), {
              [Op.like]: `%${queryLower}%`
